...
export default function SuggestArea({ modal, changeModal }: Iprops) {
  ...
  return <div className={`${isHidden} ${style.formBox}`}>
    <form ref={form} className={style.form} onSubmit={sendEmail}>
      ...
      <input type='text' id="user_name" name="user_name" value={inputs.user_name} onChange={handleChange} minLength={2} maxLength={30}></input>
      <label htmlFor='user_email'>Email:</label>
      <input type='email' id='user_email' name='user_email' value={inputs.user_email} onChange={handleChange}></input>
      <label htmlFor='lng'>Message:</label>
      <textarea id='message' name='message' value={inputs.message} onChange={handleChange} placeholder='To suggest an area, please include name and GPS coordinates' minLength={20} maxLength={2000}/>
